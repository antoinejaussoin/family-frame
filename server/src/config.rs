...
pub struct Config {
    ...
    /// Positive = woke late. Written automatically from timer polls; capped at ±50%.
    #[serde(default)]
    pub pico_drift: f64,
...
"#,
    ...
    #[test]
    fn pico_drift_over_fifty_percent_is_clamped_on_load() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("config.toml");
        std::fs::write(&path, "pico_drift = 0.6\n").unwrap();
        let cfg = Config::load(&path).unwrap();
        assert_eq!(cfg.pico_drift, 0.5);
    }
